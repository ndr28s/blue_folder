...
export class ElectronCapacitorApp {
  ...
  private async loadMainWindow(thisRef: any) {
    if (thisRef.serverUrl) {
      await thisRef.MainWindow.loadURL(thisRef.serverUrl);
    } else {
  ...
  async init(): Promise<void> {
    ...
    this.MainWindow.webContents.setWindowOpenHandler((details) => {
      const allowed = details.url.includes(this.customScheme) || details.url.startsWith('http://localhost');
      return allowed ? { action: 'allow' } : { action: 'deny' };
    });
    this.MainWindow.webContents.on('will-navigate', (event, newURL) => {
      const currentURL = this.MainWindow.webContents.getURL();
      const isAllowed =
        currentURL.includes(this.customScheme) ||
    ...
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': [
          electronIsDev
            ? `default-src ${customScheme}://* http://localhost:* ws://localhost:* 'unsafe-inline' devtools://* 'unsafe-eval' data: blob:`
            : `default-src ${customScheme}://* http://localhost:* ws://localhost:* 'unsafe-inline' data: blob:`,
        ],
      },
    });
  });
